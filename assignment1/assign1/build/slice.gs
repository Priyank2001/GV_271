...
int clipTri(vec3 p0,vec3 n0,vec3 p1,vec3 n1,vec3 p2,vec3 n2, vec3 planeN,
            out vec3 outP[6], out vec3 outN[6]){
    float d0=dot(p0,planeN), d1=dot(p1,planeN), d2=dot(p2,planeN);
...
void main(){
    ...
    int nA = clipTri(p0,n0,p1,n1,p2,n2, uWedgeNormalA, stageAP, stageAN);

    for(int t = 0; t < nA; t++){
        vec3 stageBP[6]; vec3 stageBN[6];
        int nB = clipTri(stageAP[t*3+0],stageAN[t*3+0], stageAP[t*3+1],stageAN[t*3+1],
                          stageAP[t*3+2],stageAN[t*3+2], uWedgeNormalB, stageBP, stageBN);
        for(int u = 0; u < nB; u++){
